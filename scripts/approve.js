...
async function main() {
  ...
  for (let account of accounts) {
    await erc20TokenContract.transfer(account.address, 100000);
    await erc20TokenContract.connect(account).approve(zkRollupContract.address, 100000);
    await zkRollupContract.connect(account).resigter(1, 2, 100);
  }
