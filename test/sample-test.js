...
describe("Verifier testing", function () {
  ...
  before(() => {
    ...
    state = JSON.parse(fs.readFileSync("./tx.json").toString());
  });
  ...
  it("Should veirfy success", async function () {
    ...
    for (let i = 0; i < state["number_tx"]; ++i) {
      ...
      let nonce = utils.hexlify(
        toBigNumber(state["tx" + i]["sender"]["nonce"])
      );
      ...
      transaction_inputs += sender_address.slice(2) + receiver_address.slice(2) + amount.slice(2) + nonce.slice(2)
      //console.log(transaction_inputs);
    }

    const instance = await Main.deploy(proof.input[input.length - 1]);
    await instance.deployed();
    await instance.verifyTx(inVk, vkGammaABC, inProof, transaction_inputs, input[3], input[0]);
//    console.log(transaction_inputs);
  ///  console.log(output);
  });
});
